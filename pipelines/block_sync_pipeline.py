# Bitcoin fullnode -> ClickHouse synchronization pipeline
## 1. ETL sync the data from Bitcoin node to ClickHouse (blocks_fat and transactions_fat tables)

# Preparation steps:
## 2. Run missing block detection to find missing blocks_fat in ClickHouse
## 3. Patch missing blocks by re-exporting them from Bitcoin node to ClickHouse
## 4. Run missing transaction_fat detection to find missing transactions in ClickHouse
## 5. Patch missing transactions by re-exporting them from Bitcoin node to ClickHouse
## 6. Deduplication of blocks_fat and transactions_fat table

# Sync data in the clickhouse
## 7. Sync blocks_fat and transactions_fat tables to blocks and transactions tables
## 8. Sync inputs and outputs table with data from transactions table
## 9. update outputs with the spending info in the inputs table

# Sync data to the Nabula
## 10. Export data from ClickHouse to Nabula


## 2: Missing Block Detection and Recovery
### Process:
### 1. Query ClickHouse to find the highest block number (blocks_largest_number) in blocks table
### 2. Compare with blocks_fat table:
###    - If equal: Wait for 3 minutes for new blocks
###    - If less: Proceed to block validation
### 3. Validate block (blocks_largest_number + 1):
###    - Check if block exists in blocks_fat
###    - Verify all transactions for this block are present
### 4. Recovery action:
###    - If missing: Patch blocks and transactions
###    - Run deduplication process
###    - If complete: Run deduplication only
...
def validate_block_transactions(clickhouse_client, block_number):
    """
    Validate if all transactions for a specific block exist in transactions_fat table
    Returns: List of missing transactions
    """
    query = f"""
    WITH flattened AS (
        SELECT
...
def detect_and_recover_missing_blocks(clickhouse_client):
    """
    Detect and recover missing blocks by comparing blocks and blocks_fat tables
    """
    try:
        # 1. Get highest block numbers from both tables
        blocks_max_query = "SELECT MAX(number) as max_num FROM blocks"
        blocks_fat_max_query = "SELECT MAX(number) as max_num FROM blocks_fat"
        
        blocks_max = clickhouse_client.execute(blocks_max_query)[0][0]
        blocks_fat_max = clickhouse_client.execute(blocks_fat_max_query)[0][0]

        # 2. Compare block numbers
        if blocks_max >= blocks_fat_max:
            print("Blocks are in sync. Waiting 3 minutes...")
            time.sleep(180)  # Wait 3 minutes
            return False

        # 3. Validate next block using new validation function
        next_block_num = blocks_max + 1
        block_fat_exists = f"SELECT COUNT(*) FROM blocks_fat WHERE number = {next_block_num}"
        missing_block = clickhouse_client.execute(block_fat_exists)[0][0] == 0
        missing_transactions = validate_block_transactions(clickhouse_client, next_block_num)
        
        # 4. Recovery actions
        if missing_block or missing_transactions:
            # Block has missing transactions
            print(f"Block {next_block_num} has {len(missing_transactions)} missing transactions")
            
            # Patch missing block and its transactions
            fix_block_hole([next_block_num])
            return False
        else:
            return True
        ...
        print(f"Error in block detection/recovery: {str(e)}")
        return False
...
def copy_transactions_from_fat(client, block_number, partition):
    ...
    SELECT *
    FROM bitcoin.transactions_fat final
    WHERE block_number = {block_number}
    AND toYYYYMM(block_timestamp) = {partition}
    """
    client.command(query)
...
def main():
    ...
    while detect_and_recover_missing_blocks(client):
        try:
