...
def populate_inputs_outputs_split(client, partition):
    # insert inputs_outputs
    insert_inputs_outputs_sql = f"""
    INSERT INTO inputs_outputs
    SELECT
        ...
        o.value AS o_value,
        o.is_coinbase AS o_is_coinbase,
        if(o.transaction_hash = '', 0, 1) AS revision 
    FROM inputs AS i
    ...
    """
    client.command(insert_inputs_outputs_sql)
    print(f"-- Populating inputs outputs split for partition {partition}...")

def check_ios_inputs_count_equal(client, partition: int) -> bool:
    """
    Check if the count of inputs_outputs matches the count of inputs for a given partition.
...
def inputs_outputs_remaining_count(client, partition: int) -> bool:
    """
    Check if inputs_outputs table has revision 0 for a given partition.
    ...
    Returns:
        bool: True if revision 0 exists, False otherwise.
    """
    query = f"""
        SELECT count() 
        FROM inputs_outputs
...
def inputs_remaining_count(client, partition: int) -> int:
    """
    Get the count of inputs_remaining for a given partition.
    ...
    Returns:
        int: Count of inputs_remaining.
    """
    query = f"""
        SELECT count() 
        FROM inputs_remaining
        WHERE toYYYYMM(block_timestamp) = {partition}
    """
    result = client.query(query)
    return result.result_rows[0][0]
...
def create_inputs_remaining_table(client):
    create_sql = f"""
    CREATE TABLE IF NOT EXISTS inputs_remaining 
    (
...
def populate_inputs_remaining(client, partition):
    # Finalize spent info update
    insert_spent_sql = f"""
    INSERT INTO inputs_remaining
    SELECT
    ...
    print(f"-- Populating inputs_remaining for partition {partition}...")
...
def populate_outputs_by_ios(client, partition):
    # Finalize spent info update
    insert_spent_sql = f"""
    INSERT INTO outputs
    SELECT
...
def populate_inputs_by_ios(client, partition):
    # Finalize spent info update
    insert_spent_sql = f"""
    INSERT INTO inputs
    SELECT
...
def main():
    ...
    for partition in partitions:
        try:
            ...
            populate_inputs_outputs_split(client, partition)
            if not check_ios_inputs_count_equal(client, partition):
                print(f"❌ Inputs count does not match outputs count for partition {partition}.")
                return

            populate_inputs_remaining(client, partition)
            ios_remain_count = inputs_outputs_remaining_count(client, partition)
            inputs_remain_count = inputs_remaining_count(client, partition)
            if ios_remain_count != inputs_remain_count:
                print(f"❌ Inputs remaining count {inputs_remain_count} does not match inputs_outputs revision 0 count for partition {partition}.")
                return

            populate_inputs_outputs(client, partition)
            populate_inputs_by_ios(client, partition)
            populate_outputs_by_ios(client, partition)
