...
def has_duplicate_rows(client, partition):
    ...
    (
        ...
        GROUP BY transaction_hash, output_index
        HAVING count() > 1
        LIMIT 1
    )
    """
...
def optimize_partition_final(client, partition):
    print(f"--- Optimizing partition {partition} in outputs ...")
    query = f"OPTIMIZE TABLE outputs PARTITION '{partition}' FINAL"
    client.command(query)
    print(f"✅ Partition {partition} in outputs optimized.")

def main():
    parser = argparse.ArgumentParser(description="Deduplicate partitions in outputs from a start partition onward")
        ...
        while True:
            try:
                if not has_duplicate_rows(client, partition):
                    print(f"✓ Partition {partition} in outputs is fully optimized. Skipping.")
                    break
