...
def truncate_partition(client, partition):
    print(f"Truncating partition {partition} in inputs_outputs...")
    query = f"ALTER TABLE inputs_outputs DROP PARTITION '{partition}'"
    client.command(query)
    print(f"✅ Partition {partition} truncated in inputs_outputs.")

def main():
    parser = argparse.ArgumentParser(description="Deduplicate partitions in inputs_outputs from a start partition onward")
