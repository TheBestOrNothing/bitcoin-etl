...
def drop_partition_from_inputs(client, partition):
    try:
        query = f"ALTER TABLE inputs DROP PARTITION {partition}"
        client.query(query)
        print(f"--- Successfully dropped partition {partition} from 'inputs' table.")
    except Exception as e:
        print(f"❌ Failed to drop partition {partition} from 'inputs' table: {e}")

def refill_inputs_partition(client, partition: str):
    query = f"""
    INSERT INTO inputs
    SELECT
    ...
    LEFT JOIN outputs AS o
      ON input.2 = o.transaction_hash
     AND input.3 = o.output_index
    WHERE toYYYYMM(t.block_timestamp) = {partition}
      AND toYYYYMM(o.block_timestamp) <= {partition}
    """
    client.query(query)
    print(f"--- Successfully refill partition {partition} to 'inputs' table.")
    ...
    for partition in partitions:
        try:
            ...
            if results:
                ...
                drop_partition_from_inputs(client, partition)
                refill_inputs_partition(client, partition)
