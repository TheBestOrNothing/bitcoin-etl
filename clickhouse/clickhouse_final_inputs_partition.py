...
def optimize_partition_final(client, partition):
    print(f"--- Optimizing partition {partition} in inputs ...")
    query = f"OPTIMIZE TABLE inputs PARTITION '{partition}' FINAL"
    client.command(query)
    print(f"✅ Partition {partition} in inputs optimized.")
        ...
        while True:
            try:
                if not has_duplicate_rows(client, partition):
                    print(f"✓ Partition {partition} in inputs is fully optimized. Skipping.")
                    break
